...
export default function QuickViewModal({ product, onClose }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="relative w-[92vw] max-w-5xl rounded-3xl border border-[#0A2238]/20 bg-[#EBEBEB] p-4 md:p-6 text-[#0A2238]">
        <button
          onClick={onClose}
          className="absolute top-3 right-3 p-2 rounded-xl bg-[#0A2238] border border-[#0A2238] hover:bg-[#0A2238]/80"
          aria-label="Close"
        >
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            <div className="relative aspect-[4/3] w-full rounded-2xl overflow-hidden border border-[#0A2238]/20 bg-white">
              <Image src={gallery[0]} alt={product.name} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-contain p-6" />
            </div>
            <div className="grid grid-cols-4 gap-3">
              {gallery.slice(1,5).map((src, i) => (
                <div key={i} className="relative aspect-square rounded-xl overflow-hidden border border-[#0A2238]/20 bg-white">
            ...
            {product.specs ? (
              <div className="mt-6 grid grid-cols-2 gap-3">
                {Object.entries(product.specs).slice(0,8).map(([k,v]) => (
                  <div key={k} className="rounded-xl border border-[#0A2238]/20 bg-white px-3 py-2">
                    <div className="text-[#0A2238]/60 text-xs">{k}</div>
                    <div className="font-medium text-[#0A2238]">{String(v)}</div>
                  </div>
                ))}
              </div>
            ) : null}
            ...
            {HAS_WHATSAPP_NUMBER && (
              <div className="mt-6">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-[#0A2238] text-white font-semibold hover:bg-[#0A2238]/90 transition-all duration-200 hover:scale-105"
                >
